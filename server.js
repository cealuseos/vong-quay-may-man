...
function body(req) { return new Promise((resolve, reject) => { let raw = ''; req.on('data', c => { raw += c; if (raw.length > 5_000_000) req.destroy(); }); req.on('end', () => { try { resolve(raw ? JSON.parse(raw) : {}); } catch { reject(new Error('Dữ liệu không hợp lệ')); } }); }); }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/api/admin/save-images' && req.method === 'POST') { const d = await body(req), s = state(); if(d.prizeImage) s.prizeImage = d.prizeImage; if(d.promoImage) s.promoImage = d.promoImage; if(d.promoUrl !== undefined) s.promoUrl = d.promoUrl; save(s); return json(res, 200, s); }
